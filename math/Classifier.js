...
export class Classifier {
  ...
  updateCameraDisplay(SystemState, test) {
    ...
    if (SystemState.video.readyState === SystemState.video.HAVE_ENOUGH_DATA) {
      ...
      SystemState.videoDisplayCtx.drawImage(
        SystemState.video,
        0,
        0,
        SystemState.videoDisplayCanvas.width,
        SystemState.videoDisplayCanvas.height,
      );
      // 方式 B: 获取 ImageData 并修改像素 (性能较低，但更灵活)
      const imageData = SystemState.videoDisplayCtx.getImageData(
        0,
        0,
        SystemState.videoDisplayCanvas.width,
        SystemState.videoDisplayCanvas.height,
  ...
  processImageFromCamera(data, width, height) {
    ...
    const BLUE_MIN_VALUE = 0.0; // 最小亮度 (偏暗)
    ...
    const RED_MIN_SATURATION = 0.7; // 最小饱和度
    const RED_MIN_VALUE = 0.0; // 最小亮度 (偏暗)
    ...
    function isBlue(r, g, b) {
      ...
      // 蓝色基础条件
      const hueMatchBlue =
        Math.abs(hsv.h - BLUE_HUE) <= BLUE_HUE_TOLERANCE ||
      ...
      const hueDiffRed = Math.min(
        Math.abs(hsv.h - RED_HUE),
        Math.abs(hsv.h - 360),
      );
    ...
    function searchColorInGrid(
      ...
      for (let r = 0; r <= radius; r++) {
        ...
        for (let d = 0; d < 8; d++) {
          ...
          if (
            r === 0 ||
            !boundaryGrids.some((g) => g.gx === gx && g.gy === gy)
          ) {
        ...
        for (const { gx, gy } of boundaryGrids) {
          ...
          if (
            gx < 0 ||
            gx >= Math.ceil(width / gridWidth) ||
            gy < 0 ||
            gy >= Math.ceil(height / gridHeight)
          ) {
            continue;
          }
    ...
    const MAX_SEARCH_RADIUS =
      Math.max(Math.ceil(width / GRID_WIDTH), Math.ceil(height / GRID_HEIGHT)) /
      2;
    ...
    const primaryBlue = searchColorInGrid(
      centerX,
      centerY,
      MAX_SEARCH_RADIUS,
      isBlue,
    ...
    const primaryRed = searchColorInGrid(
      primaryBlue.x,
      primaryBlue.y,
      MAX_SEARCH_RADIUS,
      isRed,
    ...
    for (const [dx, dy] of directions) {
      const farthest = findFarthestPoint(
        primaryBlue.x,
        primaryBlue.y,
        dx,
        dy,
    ...
    const Sb = bdx * bdy;
    // 8. 判断一级红蓝点距离
    const primaryDistance = distance(
      primaryBlue.x,
      primaryBlue.y,
      primaryRed.x,
      primaryRed.y,
    ...
    if (
      Sb > 64 &&
      rateThresholdRev <= bratedXdY &&
      bratedXdY <= rateThreshold
    ) {
      free = true;
    }
    ...
    if (
      ...
      Sr > 64 &&
      Sb / Sr < 2.5 &&
      rateBdBmxRmaxx / 2 < rateDrmaxxbmxBdx &&
      rateDrmaxxbmxBdx < rateBdBmxRmaxx
    ) {
      console.log("红色豁免生效");
    } else {
      free == false;
    ...
    const farthestBlueOutward = findFarthestPoint(
      bmx,
      bmy,
      blueDx,
      blueDy,
    ...
    // 12.4: 外延最远点距离
    const drbr = distance(
      farthestBlueOutward.x,
      farthestBlueOutward.y,
      farthestRedOutward.x,
      farthestRedOutward.y,
    );
    ...
    const farthestBlueInward = findFarthestPoint(
      bmx,
      bmy,
      blueInwardDx,
      blueInwardDy,
    ...
    const farthestRedInward = findFarthestPoint(
      rmx,
      rmy,
      redInwardDx,
      redInwardDy,
    ...
    // 12.7: 内部最远点距离
    const dribr = distance(
      farthestBlueInward.x,
      farthestBlueInward.y,
      farthestRedInward.x,
      farthestRedInward.y,
    );
    ...
    const blueFarthestDistance = distance(
      farthestBlueOutward.x,
      farthestBlueOutward.y,
      farthestBlueInward.x,
      farthestBlueInward.y,
    );
    const redFarthestDistance = distance(
      farthestRedOutward.x,
      farthestRedOutward.y,
      farthestRedInward.x,
      farthestRedInward.y,
    ...
    const ratioBlueRedFarthestDistance =
      blueFarthestDistance / (redFarthestDistance || 1);
    if (!free) {
      if (
        ratioBlueRedFarthestDistance < 0.5 ||
        ratioBlueRedFarthestDistance > 2
      ) {
    ...
    if (drbrRatio >= 1.25 && drbrRatio <= 1.75) {
      ...
      const bluePerpDistance = distance(
        bluePerpUp.x,
        bluePerpUp.y,
        bluePerpDown.x,
        bluePerpDown.y,
      ...
      const redPerpDistance = distance(
        redPerpUp.x,
        redPerpUp.y,
        redPerpDown.x,
        redPerpDown.y,
  ...
  processImageFromCamera1(data, width, height) {
    ...
    function isColorMatch(r, g, b, target) {
      return (
        Math.abs(r - target.r) <= target.tolerance &&
    ...
    function searchMaxPointsIn8Dirs(startX, startY, targetColor) {
      ...
      for (let d = 0; d < dirs.length; d++) {
        ...
        while (isContinuous) {
          ...
          // 获取当前像素颜色
          const index =
            (Math.floor(currentY) * width + Math.floor(currentX)) * 4;
          const r = data[index];
    ...
    console.log(
      "网格搜索顺序:",
      searchGrids.map((g) => `(${g.gx},${g.gy})`).join(" -> "),
    );
      ...
      for (let y = Math.floor(startY); y < Math.floor(endY); y += 2) {
        for (let x = Math.floor(startX); x < Math.floor(endX); x += 2) {
          ...
          if (isColorMatch(r, g, b, redColor)) {
            ...
            console.log(
              `找到一级红色点: (${x},${y}) 距离蓝色点: ${distance.toFixed(2)}px`,
            );
            break redGridLoop;
          }
        }
    ...
    // --------------------------
    // 二级蓝色点（一级蓝色点的8方向最远点）
    const secondaryBlues = searchMaxPointsIn8Dirs(
      primaryBlue.x,
      primaryBlue.y,
      blueColor,
    );
    ...
    const bdy = bmaxy - bminy;
    console.log(
      `蓝色点统计 - 边界: (${bminx},${bminy})至(${bmaxx},${bmaxy}), 中心: (${bmx.toFixed(2)},${bmy.toFixed(2)}), 宽: ${bdx}, 高: ${bdy}`,
    );
    ...
    console.log(
      `蓝色中心到一级红色点距离: ${distanceBmR.toFixed(2)}px, 2倍max(bd): ${2 * maxBd}`,
    );
    ...
    // --------------------------
    // 二级红色点（一级红色点的8方向最远点）
    const secondaryReds = searchMaxPointsIn8Dirs(
      primaryRed.x,
      primaryRed.y,
      redColor,
    );
    ...
    const rdy = rmaxy - rminy;
    console.log(
      `红色点统计 - 边界: (${rminx},${rminy})至(${rmaxx},${rmaxy}), 中心: (${rmx.toFixed(2)},${rmy.toFixed(2)}), 宽: ${rdx}, 高: ${rdy}`,
    );
    ...
    const farRedInBrDir = searchMaxPointsIn8Dirs(rmx, rmy, redColor).find(
      (p) => Math.abs(p.dir - 3) < 1,
    ); // 沿x正方向（近似蓝→红方向）
    console.log(
      `红色中心外延最远点: (${farRedInBrDir?.x.toFixed(2)},${farRedInBrDir?.y.toFixed(2)})`,
    );

    // 三、红色中心→蓝色中心方向，蓝色中心外延的最远蓝色点
    const dirRb = { dx: bmx - rmx, dy: bmy - rmy }; // 红→蓝方向向量
    const farBlueInRbDir = searchMaxPointsIn8Dirs(bmx, bmy, blueColor).find(
      (p) => Math.abs(p.dir - 2) < 1,
    ); // 沿x负方向（近似红→蓝方向）
    console.log(
      `蓝色中心外延最远点: (${farBlueInRbDir?.x.toFixed(2)},${farBlueInRbDir?.y.toFixed(2)})`,
    );

    // 四、最远蓝红外延点距离
    const drbr =
      farRedInBrDir && farBlueInRbDir
        ? getDistance(
            farBlueInRbDir.x,
            farBlueInRbDir.y,
            farRedInBrDir.x,
    ...
    // 五、蓝色中心→红色中心方向的最远蓝色点
    const farBlueInBrDir = searchMaxPointsIn8Dirs(bmx, bmy, blueColor).find(
      (p) => Math.abs(p.dir - 3) < 1,
    );
    // 六、红色中心→蓝色中心方向的最远红色点
    const farRedInRbDir = searchMaxPointsIn8Dirs(rmx, rmy, redColor).find(
      (p) => Math.abs(p.dir - 2) < 1,
    );
    // 七、内部最远点距离
    const dribr =
      farBlueInBrDir && farRedInRbDir
        ? getDistance(
            farBlueInBrDir.x,
            farBlueInBrDir.y,
            farRedInRbDir.x,
    ...
    console.log(
      `蓝→红向量与x轴夹角: ${angleRad.toFixed(4)}rad (${angleDeg.toFixed(2)}°)`,
    );
    ...
    console.log(
      `蓝色最远点距离: ${blueMaxDist.toFixed(2)}, 红色: ${redMaxDist.toFixed(2)}, 比值: ${distRatio.toFixed(4)}`,
    );
    ...
    console.log(
      `红蓝中心的中心点: (${centerOfCenters.x.toFixed(2)},${centerOfCenters.y.toFixed(2)})`,
    );
    ...
    if (drbrRatio >= 1.5 && drbrRatio <= 2) {
      ...
      console.log(
        `蓝色rz1: ${rz1.toFixed(4)}, arccos: ${arccosRz1.toFixed(4)}rad`,
      );
      console.log(
        `红色rz2: ${rz2.toFixed(4)}, arccos: ${arccosRz2.toFixed(4)}rad`,
      );
    } else {
