...
export class Object {
  ...
  commitPhysicsState() {
    ...
    // 5. [核心] 物理冷重启：基于当前几何重建整个 physicsState
    // 这会清除所有 solver 历史、约束缓存、迭代残差
    // 效果：物体像"第一次出现"一样，无任何残余应力
    this._coldRestartPhysics();

    if (this.verbose) {
      console.log('[Object] commitPhysicsState complete (physics cold restart)');
    }
  }
  // [物理冷重启] 基于当前几何状态重建整个 physicsState
  // 清除所有 solver 历史、约束缓存、迭代残差
  // 保留：Object.center, Object.quaternion, constructionPoints 位置
  _coldRestartPhysics() {
    // 保存当前物理配置
    const physicsConfig = {
      physicsModel: this.physics?.model || 'force',
      stiffness: this.physics?.stiffness || 1.0,
      damping: this.physics?.damping || 0.5
    };

    // 重建物理拓扑（从当前 constructionPoints 位置）
    // 这会：
    // 1. 创建新的 particles 数组（位置来自 constructionPoints）
    // 2. 创建新的 constraints 数组（restLength 基于当前位置）
    // 3. 更新 _referenceShape（基于当前位置的局部坐标）
    // 4. 清除所有 solver 状态
    const result = this.rebuildPhysicsTopology(physicsConfig);

    if (result) {
      console.log(`[COLD RESTART] Physics rebuilt: ${result.particles} particles, ${result.constraints} constraints`);

      // [关键] 重新初始化 COM 锚点，确保质心分离逻辑正常工作
      const physicsState = this.representation.physicsState;
      if (physicsState && physicsState.particles) {
        let totalMass = 0;
        for (const p of physicsState.particles) {
          totalMass += p.mass || 0;
        }
        // 使用 this.center 作为锚点（与 commitPhysicsState 保持一致）
        physicsState.totalMass = totalMass;
        physicsState.anchorCOM = { x: this.center.x, y: this.center.y, z: this.center.z };
        physicsState.initialCOM = { x: this.center.x, y: this.center.y, z: this.center.z };
        console.log(`[COLD RESTART] COM reinitialized: anchorCOM=(${this.center.x.toFixed(3)}, ${this.center.y.toFixed(3)}, ${this.center.z.toFixed(3)})`);
      }
    }
  }
  ...
  _bindPhysicsData(points) {
    for (const p of points) {
      if (!p._physicsData) {
        ...
        // 更新 position 和 prevPosition（冷重启时重置速度为零）
        p._physicsData.position.x = p.x;
        p._physicsData.position.y = p.y;
        p._physicsData.position.z = p.z;
        p._physicsData.prevPosition.x = p.x;
        p._physicsData.prevPosition.y = p.y;
        p._physicsData.prevPosition.z = p.z;
      }
    }
  }
